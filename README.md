...
默认配置位于`config_model.py`文件中，里面已经写了详细的注释。
...
初次运行时，会通过环境变量或在项目根目录中配置的 `.env` 文件读取配置，并保存到 `config/` 目录下的json文件中。如果没有配置对应的变量，则会使用 `config.py`中的默认值。例如`os.getenv('HOST', '0.0.0.0')`，如果没有配置 `HOST` 变量，则 `HOST` 默认为`0.0.0.0`。
...
如需重新根据环境变量生成配置，请删除`config/`目录下的文件，重启服务器。

### 通过json文件配置
初次启动服务器前，先运行 `python config.py` 在`config/`目录下生成默认的json配置文件，根据`config_model.py`中的定义的格式修改即可。
...
## 通过命令行运行扫描 / 搜索
现在可以通过 `python scan.py` 直接运行扫描而不必启动服务器。请在运行扫描前确认已配置了正确的扫描路径与忽略文件夹。

此外，当前也实现了一个简易的命令行搜索接口，可以通过以下方式运行：
```bash
python search.py image <keywords>
python search.py video <keywords>
```
