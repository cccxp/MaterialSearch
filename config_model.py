...
class ModelConfigModel(BaseConfigModel):
    ...
    """
    language: Literal['Chinese', 'English'] = Field('Chinese', description='模型搜索时用的语言，可选：Chinese/English')
    name: str = Field('openai/clip-vit-base-patch32', description='CLIP模型')
    textModelName: str = Field('IDEA-CCNL/Taiyi-CLIP-Roberta-102M-Chinese', description='中文模型，需要和CLIP模型配套使用，如果MODEL_LANGUAGE为English则忽略此项')
    device: Literal['cpu', 'cuda', 'mps'] = Field('cpu', description='推理设备，cpu/cuda/mps，建议先跑benchmark.py看看cpu还是显卡速度更快。因为数据搬运也需要时间，所以不一定是GPU更快')
    textDevice: Literal['cpu', 'cuda', 'mps'] = Field('cpu', description='text_encoder使用的设备，如果MODEL_LANGUAGE为English则忽略此项')


class SearchConfigModel(BaseConfigModel):
    """ 搜索配置
    """
    cacheSize: int = Field(64, description='搜索缓存条目数量，表示缓存最近的n次搜索结果，0表示不缓存。缓存保存在内存中。图片搜索和视频搜索分开缓存。重启程序或扫描完成会清空缓存，或前端点击清空缓存（前端按钮已隐藏）')
    maxResultNum: int = Field(150, description='最大搜索出来的结果数量，如果需要改大这个值，目前还需要手动修改前端代码（前端代码写死最大150）')  # TODO: 修改前端
